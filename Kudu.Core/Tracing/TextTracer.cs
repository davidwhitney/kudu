    ...
    public class TextTracer : ITracer
    {
        ...
        private class LogFileHelper
        {
            ...
            private bool IsInfo(string type, IDictionary<string, string> attributes)
            {
                ...
                {
                    string url;
                    if (attributes.TryGetValue("url", out url))
                    {
                        // /logstream is verbose only
                        if (url.StartsWith("/logstream", StringComparison.OrdinalIgnoreCase))
                        {
                            return false;
                        }

                        // /deployments is verbose only
                        if (url.StartsWith("/deployments", StringComparison.OrdinalIgnoreCase))
                        {
                            return false;
                        }
